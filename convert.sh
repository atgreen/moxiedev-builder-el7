...
for target in moxie-elf moxiebox moxie-rtems; do
  ...
  perl -p -i -e 's/^Depends.*/Depends: moxielogic-'${target}'-binutils , moxielogic-'${target}'-newlib , moxielogic-moxie-elf-gcc-fsf-man-pages/' moxielogic-${target}-gcc*/debian/control
  ...
  for tool in binutils gdb gdb-sim; do
